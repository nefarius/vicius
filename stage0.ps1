...
Get-AppVeyorArtifacts -Account "nefarius" -Project "vicius" -Path $Path -Token $Token -Branch $BuildVersion -JobName "Platform: ARM64"
# Download x64 binaries
Get-AppVeyorArtifacts -Account "nefarius" -Project "vicius" -Path $Path -Token $Token -Branch $BuildVersion -JobName "Platform: x64"
# Download x86 binaries
Get-AppVeyorArtifacts -Account "nefarius" -Project "vicius" -Path $Path -Token $Token -Branch $BuildVersion -JobName "Platform: x86"

# List of files to sign
$files =    "`".\artifacts\bin\x64\*.exe`" " +
            "`".\artifacts\bin\ARM64\*.exe`" " +
            "`".\artifacts\bin\x86\*.exe`" "

if ($NoSigning -eq $false) {
    # sign with only one certificate
    Invoke-Expression "& `"$signTool`" sign /v /as /n `"$certName`" /a /tr $timestampUrl /fd sha256 /td sha256 $files"
}
