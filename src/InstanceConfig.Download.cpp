...
[[nodiscard]] bool models::InstanceConfig::GetReleaseDownloadStatus(
	bool& isDownloading, bool& hasFinished, int& statusCode) const
{
	if (!downloadTask.has_value())
	{
		return false;
...
void models::InstanceConfig::ResetReleaseDownloadState()
{
	downloadTask.reset();
}
